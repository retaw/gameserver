...
public:
    ...
    bool empty() const
    {
        return m_data[m_begin.load(std::memory_order_relaxed)].status.load(std::memory_order_relaxed) == Cell::Status::empty;
    }

    bool full() const
    {
        return m_data[m_end.load(std::memory_order_relaxed)].status.load(std::memory_order_relaxed) == Cell::Status::full;
    }
